...
(
   ...
   rule_set_key         varchar(50) not null comment '规则集key(package)',
   rule_enter_type      varchar(10) not null default 'SELECT' comment '规则录入方式(CODING:编码式录入; SELECT:选择式录入 )',
   rule_condition_text  varchar(1000) not null default '' comment '规则condition代码(CODING方式)',
   rule_action_text     varchar(1000) not null default '' comment '规则action代码(CODING方式)',
   rule_import_text     varchar(300) not null default '' comment '规则import内容代码(CODING方式)',
...
(
   ...
   attr_type            varchar(20) not null comment '属性类型(STRING,NUMBER,BOOLEAN,DATE,COLLECTION,OBJECT)',
   attr_model_key_ref   varchar(50) not null default '' comment '属性模型(适用于OBJECT类型属性)',
   attr_value_source    varchar(20) not null comment '属性值来源(INPUT,DICT,ADDRESS)',
   is_delete            tinyint(1) unsigned not null default 0 comment '是否删除',
   create_time          timestamp not null default CURRENT_TIMESTAMP comment '创建时间',
   update_time          timestamp not null default CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP comment '更新时间',
   primary key (id),
...
(
   ...
   value_calculate_formula varchar(300) not null default '' comment '动作值计算公式(CALCULATE 类型下有效, 计算公式中的变量,如果嵌套类的属性,使用: 010  进行分割)',
